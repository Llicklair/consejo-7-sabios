...
def _consensus_system_prompt(sage: Sage) -> str:
    return (
        ...
        f"so EVERY path in `files_touched` must be one you confirmed exists with "
        f"Read/Glob, OR explicitly a NEW file to create (mark which) — a path you "
        f"assumed but never opened is a hallucination the verifier will refute; an "
        f"item with no `files_touched` is not actionable), `amend` existing ones (rewrite a "
        ...
        f"in one directory is a slice, not a plan for the project. **When you DO "
        f"propose in a zone you have not been working in, Glob/Read a REAL file "
        f"there FIRST** — a breadth item must point at a file you actually opened, "
        f"never a path you assumed exists. Guessing `billing.ts` or `UsageMeter."
        f"tsx` into existence is the exact failure the verifier refutes; spend one "
        f"of your tool calls to ground the proposal in that zone before making it.\n"
...
def _consensus_turn_user_message(
    ...
    brief_block = (
        ...
        f"symbols, internal wiring (who imports whom), hubs, import cycles, and a "
        f"debate-priority score (blast-radius + coupling + size + churn). This is "
        f"your ground truth: propose from it, not from guesses. The FULL per-file "
        f"map is at `.consejo/repo-skeleton.md` — Read it ONCE to ground yourself, "
        f"start from the highest-score files, then Read those specific files (max "
        f"3 calls) to judge their logic.\n\n"
        f"{repo_brief}\n"
        f"</repo_analysis>\n\n"
    ) if repo_brief else ""
    return (
